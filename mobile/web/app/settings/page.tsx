    ...
    try {
      if (mode === "login") {
        ...
        if (res.demo_code) {
          ...
          setCode(res.demo_code);   // auto-fill the code input
          setMode("reset");
        }
      } else if (mode === "reset") {
...
function UsersPanel({ me }: { me: UserOut | null }) {
  ...
  async function create() {
    ...
    setCreateBusy(true);
    try {
      // Save admin token before register() overwrites localStorage with the new user's token
      const adminToken = typeof window !== "undefined" ? localStorage.getItem("vc_token") : null;
      const res = await api.register(newEmail, newPw, newName || undefined);
      // Restore admin token so the admin stays logged in
      if (adminToken && typeof window !== "undefined") localStorage.setItem("vc_token", adminToken);
