...
export default function StatusPage() {
  ...
  async function check() {
    ...
    const t0 = Date.now();
    try {
      const res = await fetch(`${api.apiBase}/health`);
    ...
    // 2. Database (currencies endpoint exercises ORM)
    const t1 = Date.now();
    try {
      ...
      results.push({ name: "Database / ORM", status: "error", detail: "Query failed", latency: Date.now() - t1 });
    }

    // 3. Currency catalogue
    const t2 = Date.now();
    try {
      const c = await api.currencies();
      const enabled = c.filter((x) => x.enabled);
      results.push({
        name: "Currency catalogue",
        status: enabled.length > 0 ? "ok" : "error",
        detail: `${enabled.length} of ${c.length} enabled`,
        latency: Date.now() - t2,
      });
    } catch {
      results.push({ name: "Currency catalogue", status: "error", detail: "Unavailable", latency: Date.now() - t2 });
    }
